...
{
    int inputsteer = remoteinput->angular.z;
    if(inputsteer != 0)
        start = 1;
}
...
{
    write_speed = key->linear.x;
    write_steer = key->linear.y;
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber remote_sub_1        = nh.subscribe("/joy_teleop/cmd_vel",1,remoteCallBack);
    ros::Subscriber key_sub             = nh.subscribe("control",1,keyCallBack);
    ...
    while(ros::ok)
    {
        ...
        if (kappa>0.908) {kappa=0.908;}
         ...
         else if(write_speed<0) { write_speed = 0;}
