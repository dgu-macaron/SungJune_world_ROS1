...
void checking_danger_lane()
{
    ...
    for(int i = 0; i < 541; i++)
    {
        ...
        if (x < obstacle_checking_d && x > 0 && fabs(y) < lane_width / 2.0)
        {
        ...
        else if (x < obstacle_checking_d && x > 0 && fabs(y + lane_width*(available_path[0] - available_path[2])) < lane_width / 2.0 && !(mission_num == dy_obs_mission))
        {
            obstacle_data[2][i] = 1;
            double TEMP = x;
...
double path_cost(int lane)
{
	...
	return w_consistency*cost_consistency + w_obstacle*cost_obstacle;	
}
...
void visualizing_path(int index, int lane, int selected)
{
    for(int i = 0; i < int(candidate_path_leng); i++)
    {
        ...
        path_rviz_msg.id = i + 100 + lane * int(candidate_path_leng);
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok)
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            if(available_path[i])
            {
                generate_candidate_path(0, i);
                visualizing_path(0, i, 0);
        ...
        generate_candidate_path(0, selected_lane);
        path_cost(selected_lane);
        qf_before = qf;
        visualizing_path(0, 3, 1);
        print_on_terminal();
